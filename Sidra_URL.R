...
sidra_url = function(table, time_interval, variables, territorial_perspective, territorial_level, kind, headers, fields, decimals){
  ...
  # Header (Optional)
  if(is.null(headers)||headers == FALSE){headers = 'h/n/'} else if(headers == TRUE){headers = paste0('h/y/', header)} else {message('Opção inválida para o argumento header. Tente novamente.')}
